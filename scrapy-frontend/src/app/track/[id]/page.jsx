...
export default function TrackItem() {
  ...
  useEffect(() => {
    const fetchGraphDetails = async () => {
      try {
        // Try fetching Amazon product graph details
        const response = await axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/amazon/graph_details/${id}`);
        ...
        console.error("Error fetching Amazon graph details:", amazonError.message);
        
        // Try fetching Jumia product graph details if Amazon fails
        try {
          const response = await axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/jumia/graph_details/${id}`);
          const productDetails = response.data.products;
          ...
          console.error("Error fetching Jumia graph details:", jumiaError.message);
          setError('No graph data available for this product.');
        }
      }
    };
  ...
  useEffect(() => {
    ...
    const fetchItemData = async () => {
      ...
      try {
        // First, attempt to fetch Amazon product data
        ...
        console.error('Error fetching Amazon data:', amazonError.message);
  
        // If Amazon fetch fails, attempt to fetch Jumia product data
        try {
          const jumiaResponse = await axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/jumia/frontend_data/${id}`);
          console.log('Jumia Response:', jumiaResponse.data);
          ...
          console.error('Error fetching Jumia data:', jumiaError.message);
  ...
  useEffect(() => {
    const fetchTrackedUrl = async () => {
      try {
        // Attempt to fetch the tracked URL from Amazon
        const amazonResponse = await axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/amazon/get_tracked_url`);
        const amazonUrls = amazonResponse.data;
        const amazonUrlObj = amazonUrls.find(urlObj => urlObj.id === id);
        
        if (amazonUrlObj) {
          setProductUrl(amazonUrlObj.url);
        } else {
          // If no Amazon URL is found, try fetching from Jumia
          try {
            const jumiaResponse = await axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/jumia/get_tracked_url`);
            const jumiaUrls = jumiaResponse.data;
            const jumiaUrlObj = jumiaUrls.find(urlObj => urlObj.id === id);
  
            if (jumiaUrlObj) {
              setProductUrl(jumiaUrlObj.url);
            } else {
              console.error('No tracked URL found for the given ID.');
            }
          } catch (jumiaError) {
            console.error('Error fetching Jumia tracked URL:', jumiaError.message);
          }
        }
      } catch (amazonError) {
        console.error('Error fetching Amazon tracked URL:', amazonError.message);
      }
    };
  ...
  const handleChatClick = () => {
    ...
    const platformEndpoint = platform === 'amazon' ? 'amazon' : 'jumia';
  
    // Post the product URL and filename to the appropriate endpoint
    axios.post(`${process.env.NEXT_PUBLIC_API_BASE_URL}/${platformEndpoint}/get_comments`, {
      url: productUrl,
      file_name: filename,
    })
    .then(() => {
