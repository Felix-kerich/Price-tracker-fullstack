    ...
    try:
        while True:
            ...
            try:
                ...
                # Convert to JSON if needed and log the response
                response_to_send = prepare_response(llm_response)
                logging.info(f"Sending response: {response_to_send}")
...
def prepare_response(response):
    """Prepare the response based on the LLM output to ensure JSON format."""
    if response is None:
        logging.warning("LLM response is None, sending default message.")
        return json.dumps({"message": "No response generated."})
    ...
    if isinstance(response, str):
        ...
        try:
            ...
            logging.error(f"JSON serialization error: {e}")
            return json.dumps({"error": "Unable to serialize response to JSON."})
    else:
        return json.dumps({"message": str(response)})  # Fallback to string conversion
if __name__ == "__main__":
    import uvicorn
    uvicorn.run(app, host="127.0.0.1", port=8001)